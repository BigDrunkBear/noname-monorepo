...
type SupportedChains = ChainId.Goerli | ChainId.Hardhat ;//| ChainId.Mainnet;
...
/*export const createNetworkHttpUrl = (network: string): string => {
  const custom = process.env[`REACT_APP_${network.toUpperCase()}_JSONRPC`];
  return custom || `https://eth-goerli.g.alchemy.com/v2/${ALCHEMY_PROJECT_ID}`;
};

export const createNetworkWsUrl = (network: string): string => {
  const custom = process.env[`REACT_APP_${network.toUpperCase()}_WSRPC`];
  return custom || `wss://eth-goerli.g.alchemy.com/v2/${ALCHEMY_PROJECT_ID}`;
};*/
...
const app: Record<SupportedChains, AppConfig> = {
  [ChainId.Goerli]: {
    ...
    subgraphApiUri: 'https://api.thegraph.com/subgraphs/name/bigdrunkbear/nouns-subgraph-goerli-1',
